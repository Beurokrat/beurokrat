...
const MobileMenu: React.FC<MobileMenuProps> = ({ overlayAction, setOverlayAction, closeMobileMenu }) => {
    const handleClick = (route: string, featureName: string) => {
        trackEvent(route, { feature_name: `MOBILE : ${featureName}` })
        closeMobileMenu()
    }
                ...
                {menuItems.map((item, index) => (
                    <Link
                        ...
                        className={`self-stretch flex flex-row items-center py-[16px] border-b-[1px] ${
                            item.isButton
                                ? 'mt-[50px] px-5 py-[16px] bg-primary rounded-[35px] max-w-[182px] justify-center'
                                : 'justify-between'
                        }`}
                    >
                        {item.isButton ? (
                            <div className="cursor-pointer flex flex-row flex-wrap items-center justify-center hover:bg-greenyellow ">
                                <div
                                    className="relative text-center leading-[21px] font-cta-button-text-title-case-mob"
                                    style={{ fontSize: '15px', color: '#0A0200', fontWeight: '600' }}
                                >
                                    {item.label}
                                </div>
                            </div>
